    ...
    if (!config.cliRenderingEnabled) {
        ...
        window.registerKeyCallback([&](int key, int /* scancode */, int action, int /* mods */) {
            if (action == GLFW_PRESS) {
                ...
                case GLFW_KEY_T: {
                    // Debug for multiple rays
                    const auto tmp = window.getCursorPixel();
                    glm::vec2 mulRayPos = { tmp.x - 0.5, tmp.y - 0.5 };
                    rayList = jitterSampling((int)mulRayPos.x, (int)mulRayPos.y, screen.resolution(), scene, camera, bvh, screen, config.features, true).second;
        ...
        while (!window.shouldClose()) {
            ...
            if (viewMode == ViewMode::Rasterization) {
                ...
                ImGui::Checkbox("Draw Mul Rays", &debugMulRay);
            ...
            case ViewMode::Rasterization: {
                ...
                if (debugMulRay) {
                    //run the multiple ray function
                    enableDebugDraw = true;
                    glDisable(GL_LIGHTING);
                    glDepthFunc(GL_LEQUAL);
                    //takes list of rays from jittered sample and draws each one.
                    for (Ray ray : rayList) {
